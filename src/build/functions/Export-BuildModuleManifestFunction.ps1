function Export-BuildModuleManifestFunction {
    ...
    Write-Information "`tSet-Content -Path '$ModuleManifestPath' -Value `$UpdatedContent -Encoding UTF8 -ErrorAction Stop"
    Set-Content -Path $ModuleManifestPath -Value $UpdatedContent -Encoding UTF8 -ErrorAction Stop
}
